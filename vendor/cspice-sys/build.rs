...
fn main() {
    ...
    let cspice_dir = cspice_dir.expect(
        "Cannot build: CSPICE_DIR env var missing, no system install found, and feature \"downloadcspice\" disabled.",
    );
...
fn locate_cspice() -> Option<PathBuf> {
    match env::consts::OS {
        "linux" | "macos" if Path::new("/usr/lib/libcspice.a").exists() => Some(PathBuf::from("/usr")),
        _ => None,
    }
}
...
fn download_cspice(out_dir: &Path) {
    ...
    let (platform, extension) = match env::consts::OS {
        ...
        _ => unimplemented!(
            "Cannot fetch CSPICE source for this platform, please download manually"
        ),
    };

    let url = format!(
        "https://naif.jpl.nasa.gov/pub/naif/toolkit//C/{platform}/packages/cspice.{extension}"
    );
    ...
    let response = client
        .get(&url)
        .header(USER_AGENT, "solar_travel_calculator/0.1")
        .send()
    ...
    let body = response
        .bytes()
        .expect("Failed to read CSPICE archive");

    fs::write(&download_target, body).expect("Failed to write archive file");
    ...
    match (env::consts::OS, extension) {
        ("linux" | "macos", "tar.Z") => {
            ...
            .expect("Failed to rename cspice library");
        }
        ("windows", "zip") => {
            Command::new("tar")
                .current_dir(out_dir)
