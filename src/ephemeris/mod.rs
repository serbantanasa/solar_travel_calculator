...
pub fn state_vector(
    ...
    let reference_frame = CString::new(reference_frame).unwrap();
    let aberration_correction = CString::new(aberration_correction).unwrap();
    let epoch = CString::new(epoch).unwrap();

    let mut ephemeris_time: SpiceDouble = 0.0;
    unsafe {
        str2et_c(epoch.as_ptr() as *mut i8, &mut ephemeris_time);
    }
