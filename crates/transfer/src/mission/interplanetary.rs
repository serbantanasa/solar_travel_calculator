...
fn boundary_states(
    ...
    let arr_target_name = config
        .arrival_epoch
        .as_ref()
        .map(|_| config.destination_body.clone())
        .unwrap_or_else(|| config.destination_body.clone());
    let arr_target = ephemeris::normalize_heliocentric_target_name(&arr_target_name);
    ...
    let arrival_state =
        ephemeris::state_vector(&arr_target, "SUN", "ECLIPJ2000", "NONE", &arrival_epoch)?;
